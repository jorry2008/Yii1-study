...
class CHttpRequest extends CApplicationComponent
{
	 ...
	 * 每个组件的实例化由Module类的$component->init();提供。
	 */
	...
	{
		// normalize request
		//by jorry get_magic_quotes_gpc()
		//当magic_quotes_gpc=On的时候，函数get_magic_quotes_gpc()就会返回1
		//当magic_quotes_gpc=Off的时候，函数get_magic_quotes_gpc()就会返回0
		if(function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc())//已经自动转义了
		{
			if(isset($_GET))
		...
		//以上解决php版本问题导致的转义混乱，最终接收到纯洁的原始数据
		//将已经自动转义的数据还原
	...
	public function stripSlashes(&$data)
	{
		if(is_array($data))
		{
			if(count($data) == 0)
				return $data;
			$keys=array_map('stripslashes',array_keys($data));//by jorry将键值都删除转义
