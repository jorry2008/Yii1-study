...
abstract class CModule extends CComponent
{
	...
	public function __construct($id,$parent,$config=null)
	{
		...
		//注意：$this->getBasePath()是可读的，且利用了延时加载创建了合适的path
	...
	public function getBasePath()
	{
		...
		{
			//此$this可能是webapp，也可能是moduleapp对象
			//以反射的方式获取app的对象$this的文件所在路径
	...
	{//fb($id);
		...
		elseif(isset($this->_componentConfig[$id]) && $createIfNull)//by jorry有引入的相关对象参数，可以用来创建对象
		{
			...
			if(!isset($config['enabled']) || $config['enabled'])
			{
				...
				$component->init();
				return $this->_components[$id]=$component;//by jorry单例写入
			}
		}
	}
	...
	public function setComponent($id,$component,$merge=true)
	{
		...
		if(isset($this->_componentConfig[$id]) && $merge)
			...
			$this->_componentConfig[$id]=$component;//by jorry第一次，起初这里只加配置
	}
	...
	public function configure($config)
	{
		if(is_array($config))
		{
			foreach($config as $key=>$value) {
				//by jorry各种没定义的属性在这里产生
				//其中有一个组件属性components，它都会处理所有配置的组件！！！
				$this->$key=$value;
			}
		}
	}
	...
	public function get_componentConfig() {
