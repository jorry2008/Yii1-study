...
class YiiBase
{
	 ...
	 * @since 1.1.8
	 */
	public static $enableIncludePath=true;//by jorry这个配置从框架之外设置，即设置加载范围是否超出框架，默认ture超出，即使用系统的include_paths加载目录
	
	//by jorry 别名数组
	private static $_aliases=array('system'=>YII_PATH,'zii'=>YII_ZII_PATH); // alias => path//by jorry所有使用过别名的路径
	//重点理解：
	/**
	 * $_imports此属性将记录所有通过Yii::imports()方法引入过的且具备加载能力的类和路径！
	 * 它只是记录这个过程，
	 * 如果是强制加载则requre类即可，
	 * 如果是类引入则分配到$classMap中，
	 * 如果路径引入则设置到php的包含路径中（开启包含），
	 * 如果路径设置到$_includePaths中在框架范围内加载。
	 */
	private static $_imports=array();					// alias => class name or directory
	private static $_includePaths;						// list of include paths
	private static $_app;//by jorry 实现Yii::app()的调用方法
	...
	public static function createComponent($config)
	{
		...
		if(!class_exists($type,false))//by jorry 参数true即不调用__autoload()
			$type=Yii::import($type,true);//by jorry 强制import引入，即直接require
		...
		if(($n=func_num_args())>1)
		{
			...
			{
				unset($args[0]);
				$class=new ReflectionClass($type);//by jorry 利用反射实例化多参数对象
		...
		foreach($config as $key=>$value)//by jorry这里是yii强制的，即所有的参数都为对应对象的public属性!!
			$object->$key=$value;
	 ...
	 * 如：application.components.GoogleMap将导入 GoogleMap类，直接进入：self::$classMap
	 * 导入一个目录相当于添加一个目录到PHP包含路径中。 
	 * 如果有多个目录被导入，在类文件搜索时， 后导入的目录优先于之前导入目录（例如，它们被添加到了PHP包含路径的前面）。
	 * 如：application.components.*将导入components目录且添加到php包含路径前面，直接添加到php包含路径，如果
	 * 
	 * 注：
	 * 相同的路径别名可以多次导入，但只有首次有效。 导入一个目录将不会导入其下面的任何子目录。
	 * 1.1.5起也可以使用命令空间格式导入一个类 (仅PHP 5.3及以上版本可用)。
	 * 强调：命名空间application\components必须对应一个有效的 路径别名application.components
	 * @throws CException if the alias is invalid
	 */
	public static function import($alias,$forceInclude=false)//by jorry，$forceInclude是否强制引入，而不仅是添加到self::$_imports数组中
	{
		if(isset(self::$_imports[$alias]))  // previously imported//by jorry判断是否已经引入过此别名
			return self::$_imports[$alias];//by jorry如果已经引入过，则直接返回此别名

		if(class_exists($alias,false) || interface_exists($alias,false))
			return self::$_imports[$alias]=$alias;//class

		//by jorry处理命名空间
		if(($pos=strrpos($alias,'\\'))!==false) // a class name in PHP 5.3 namespace format
		{
			$namespace=str_replace('\\','.',ltrim(substr($alias,0,$pos),'\\'));
			if(($path=self::getPathOfAlias($namespace))!==false)//by jorry框架范围内的路径
			{
			...
			else
			{
				...
				if (class_exists($alias,true))//by jorry这是一个可加载的类，已经引入过了
		...
		{
			if($forceInclude && self::autoload($alias))//by jorry直接完成加载，利用已经有的功能处理
				self::$_imports[$alias]=$alias;
			return $alias;
		}
		...
		if(($path=self::getPathOfAlias($alias))!==false)
		{
			...
			else  // a directory
			{
				...
				array_unshift(self::$_includePaths,$path);//by jorry从头部添加一个路径

				//by jorry这里设置所有的自动加载路径：set_include_path，注意有不支持此方法的，yii会处理它
	...
	{//fb($className);
		...
		if(isset(self::$classMap[$className]))//by jorry检查第三方类
		{
			include(self::$classMap[$className]);
		}
		elseif(isset(self::$_coreClasses[$className]))//by jorry加载yii核心类
		{
			include(YII_PATH.self::$_coreClasses[$className]);//by jorry由framework目录开始
		}
		else
		{
			// include class file relying on include_path
			if(strpos($className,'\\')===false)  // class without namespace，没有使用php5.3的命名空间
			{
				if(self::$enableIncludePath===false)//by jorry设置只使用yii提供的includePaths路径，这样做，文件的加载范围强制维持在框架范围之内。
				{
					foreach(self::$_includePaths as $path)//by jorry在yii框架范围内加载
					{
					...
					include($className.'.php');//by jorry系统的set_include_paths目录和yii设置的目录自动加载
			}
			else  // class name with namespace in PHP 5.3
			{//兼容php5.3
				$namespace=str_replace('\\','.',ltrim($className,'\\'));//by jorry将php的命名空间转化为yii的命名空间
	 ...
	 * <br>Notice:Yii::getLogger()可以查看系统的执行流程
	 */
	 ...
	 * This method supports choice format (see {@link CChoiceFormat}),//指定格式，解决复数问题
	...
	public static function t($category,$message,$params=array(),$source=null,$language=null)
	{
		...
		{
			if($source===null)
				$source=($category==='yii'||$category==='zii')?'coreMessages':'messages';//by jorry 有开发者译和框架核心译两种，对应不同的类
	 ...
	 * <pre>
	 * 默认情况下，注册一个加载器，并将yii加载器放在最后面
	 * 即yii本身的优先级最低，这样做使用第三方可以使用更高的优先级对yii进行设置!!!
	 * </pre>
	...
	public static function registerAutoloader($callback, $append=false)
	{
		if($append)
		{
			self::$enableIncludePath=false;//by jorry关闭默认的系统加载器，在系统最后面添加一个第三方加载器
		...
		else
		{
			...
			spl_autoload_register($callback);
			spl_autoload_register(array('YiiBase','autoload'));//by jorry 默认降低yii加载器的优先级
		}
	}
	 ...
	 * by jorry test
	 */
	public static function get_includePaths() {
		return self::$_includePaths;
	}
	public static function get_coreClass() {
		return self::$_coreClasses;
	}
...
spl_autoload_register(array('YiiBase','autoload'));//注册yii的加载器，在以后的使用中可以

//by jorry 这里手动加载，保证了第一个加载的类，？
